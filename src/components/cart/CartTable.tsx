    ...
    return (
        <div>
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
                ...
                >
                    <span>&larr;</span> {t("continue_shopping")}
                </Link>
                <button
                    onClick={handleClearCart}
                    disabled={clearing}
                    className="text-sm text-text-muted hover:text-primary transition-colors disabled:opacity-40 cursor-pointer"
                >
                    {clearing ? "..." : t("clear_cart")}
                </button>
            </div>
        </div>
    );
